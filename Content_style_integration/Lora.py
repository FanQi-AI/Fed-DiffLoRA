...
def text_encoder_create_modules(root_module, state_dict):
    loras = []
    # 遍历 state_dict 中的键 (原始逻辑)
    # 注意：原始代码这里错误地迭代了 state_dict.items() 并使用了 mapped_name，应该只迭代 key
    for key in state_dict:
        if "lora_te" in key and ".alpha" in key:
                ...
                for child_name, child_module in module.named_modules():
                    ...
                    if child_rename == key_rename and isinstance(child_module, (nn.Linear, nn.Conv2d)):
                        try:
                            ...
                            # 注意：这里的 lora_name 比较基础，后面会在 LoRAModule 中覆盖
            ...
            #     print(f"    [原始 TE 创建函数] 警告: 找不到键 {key} 对应的目标模块 {key_rename}")
        ...
        if "lora_unet" in key and ".alpha" in key:
            ...
            for name, module in root_module.named_modules():
                ...
                if target_module_found:
                    break # 找到目标
            # if not target_module_found:
            #     print(f"    [原始 UNet 创建函数] 警告: 找不到键 {key} 对应的目标模块 {key_rename}")

    return loras

class LoRAModule(nn.Module):
    # 修改 __init__ 以处理多个 LoRA 文件，但调用原始的创建函数
    def __init__(self, text_encoder: nn.Module, unet: nn.Module, lora_state_dicts: List[Tuple[str, Dict]]):
        ...
        for lora_filename, state_dict in lora_state_dicts:
            ...
            for lora in current_te_loras:
                ...
                # print(f"      Assigned safe name: {lora.lora_name}") # 调试信息
        ...
        print(f"[LoRAModule 初始化] 完成 LoRA 层收集。总计 TE LoRA: {len(self.text_encoder_lora_modules)}, 总计 UNet LoRA: {len(self.unet_encoder_lora_modules)}")

        # 使用原始的顺序 apply_to 逻辑应用 LoRA 适配器
        print("正在向文本编码器应用 LoRA 层 (通过 apply_to 链式调用)...")
        applied_te_count = 0
        for lora in self.text_encoder_lora_modules: # 确保这里使用的是包含最终 safe name 的 lora 对象
            try:
                lora.apply_to() # 核心：链式替换 forward
                # 使用其唯一的、合法的名称添加模块，以便发现参数
                # print(f"  Adding TE module: {lora.lora_name}") # 调试信息
                self.add_module(lora.lora_name, lora)
                applied_te_count += 1
            except Exception as e:
                print(f"  错误: 向文本编码器应用 LoRA 层 {getattr(lora, 'lora_name', 'unknown')} 时出错: {e}")
        print(f"已向文本编码器应用 {applied_te_count} 个 LoRA 层。")

        print("正在向 UNet 应用 LoRA 层 (通过 apply_to 链式调用)...")
        ...
        print(f"已向 UNet 应用 {applied_unet_count} 个 LoRA 层。")


    # 这个方法结构上保持不变，应该能正常工作
    # 因为它遍历存储的 LoRALayer 实例列表。
